-- ════════════════════════════════════════════════════════════════════
--  E-LOGISTIC · 0027 · Terminy dokumentów kierowcy (compliance)
--  Prawo jazdy / Kod 95 / badania lekarskie / ADR — daty ważności + przypomnienia.
--  Daty NIE są PII wrażliwym (sama data, bez numeru) → kolumny jawne.
--  Powiadomienia BEZ nazwiska (PII szyfrowane) — generyczny tytuł „sprawdź kartotekę".
-- ════════════════════════════════════════════════════════════════════

alter table drivers add column if not exists license_expiry date;
alter table drivers add column if not exists code95_expiry  date;
...
-- ── list_drivers: dołącz daty terminów ──────────────────────────────
...
-- ── driver_save: 4 nowe parametry dat (drop+create — zmiana sygnatury) ──
drop function if exists public.driver_save(uuid, uuid, text, text, text, text[], text[], text);
...
-- ── generate_expiry_notifications: + blok dokumentów kierowcy (bez nazwiska) ──
...
begin
  ...
  -- Dokumenty kierowcy: prawo jazdy / kod 95 / badania / ADR (BEZ nazwiska — PII)
  insert into notifications (company_id, user_id, type, title, body, severity, dedup_key)

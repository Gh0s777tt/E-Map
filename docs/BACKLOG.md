<!-- SYNC: po v1.85.0 · #229 · 2026-06-28 -->
...
- [ ] **Modernizacja frontu — Tier 3 (inline-styles → klasy + CSP enforce).** Sprzężony projekt, świadomie NIE big-bang (patrz niżej):
  ...
  - **Plan (inkrementalny, per-powierzchnia, z QA):** (1) ✅ **mapa — ZROBIONE #227** (`map.module.css`, QA 1:1 computed styles); (2) ✅ **formularze `LiquidForm` — ZROBIONE #228**; (3) strony list/detali; (4) po usunięciu inline → CSP `style-src` bez `'unsafe-inline'`, potem **enforce** (z `report-uri`). shadcn/ui opcjonalnie na tym etapie.

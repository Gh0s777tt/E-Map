...
> **Czeka na najbliższy build** (w kodzie, jeszcze nie w żadnym artefakcie EAS): `[#388]` **waluta kosztu pojazdu jako wybór z listy zamiast wolnego tekstu** — wpisane `PL` zamiast `PLN` dawało wcześniej koszt zapisany poprawnie, ale nie do przeliczenia na euro, więc kwota cicho wypadała z podsumowań; podpowiedź waluty bierze się teraz z kraju firmy i jest sprawdzana wobec listy walut notowanych przez EBC. Wersja aplikacji **celowo zostaje na 1.95.0** — podbijamy ją bezpośrednio przed `eas build`, żeby nie powstał numer wersji, któremu nie odpowiada żaden artefakt.

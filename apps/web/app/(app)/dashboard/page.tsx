...
export default async function DashboardPage() {
  ...
  return (
    <div>
      ...
      {/* #296: sekcje pulpitu z możliwością ukrywania („Dostosuj") — render w RSC,
          klient tylko przełącza widoczność (preferencje w localStorage). */}
      <DashboardSections
        sections={[
          { id: "kpi", label: t("dashboard.sec.kpi"), node: <KpiStrip /> },
          { id: "trend", label: t("dashboard.sec.trend"), node: <RevenueTrend /> },
          {
            id: "onboarding",
            label: t("dashboard.sec.onboarding"),
            node: <OnboardingChecklist />,
          },
          { id: "attention", label: t("dashboard.sec.attention"), node: <AttentionPanel /> },
          {
            id: "shortcuts",
            label: t("dashboard.sec.shortcuts"),
            node: (
              <div style={{ display: "flex", gap: 16, marginTop: 24, flexWrap: "wrap" }}>
                {CARDS.map((c) => (
                  ...
                  >
                    <div style={{ color: palette.red, fontSize: 12, letterSpacing: 2 }}>
                      {t(c.tagKey)}
                    </div>

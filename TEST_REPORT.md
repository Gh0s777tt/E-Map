...
|---|---|---|---|---|---|
| ~~**Niski** (latentny)~~ **✅ ZAMKNIĘTE #223** | `packages/api/src/data/push.ts` `listPushSubscriptionsForDelivery` | Brak guardu: wywołane **bez** `companyId`/`userIds` zwracało subskrypcje **wszystkich firm** (service‑role omija RLS). **Naprawione w #223:** funkcja wymaga ≥1 filtra i **rzuca** bez niego; regresja `it.skip` → aktywny test. | (hist.) `listPushSubscriptionsForDelivery(admin)` bez opts → zwracał cały zbiór. | Teraz: rzuca „wymagany filtr…". | TAK — `push.test.ts` „bez filtra ODMAWIA" (aktywny) |

-- ════════════════════════════════════════════════════════════════════
--  E-LOGISTIC · 0040 · Dane do przelewu na fakturze (bank / nr konta)
--  companies.bank_name, companies.bank_account → snapshotowane na fakturze
--  (seller_bank/seller_account) przy wystawianiu (jak seller_name itd.).
-- ════════════════════════════════════════════════════════════════════
...
alter table invoices add column if not exists seller_bank text;
alter table invoices add column if not exists seller_account text;

-- ── create_invoice: + dane do przelewu ze snapshotu firmy ───────────
...
begin
  ...
  insert into invoices (
    ...
    seller_bank, seller_account, buyer_name, description, net, vat_rate, vat_amount,
    gross, currency, due_date
  ) values (
    o.company_id, o.id, num, comp.name, comp.tax_id, comp.address,
    comp.bank_name, comp.bank_account, o.shipper, v_desc, v_net, v_rate, v_vat,
    v_gross, coalesce(o.currency, 'EUR'), v_due
  ) returning * into inv;
...
-- ── create_blank_invoice: + dane do przelewu ze snapshotu firmy ─────
...
begin
  ...
  insert into invoices (
    company_id, order_id, number, seller_name, seller_tax_id, seller_address,
    seller_bank, seller_account, buyer_name, buyer_tax_id, buyer_address, description,
    net, vat_rate, vat_amount, gross, currency, due_date
  ) values (
    p_company, null, num, comp.name, comp.tax_id, comp.address,
    comp.bank_name, comp.bank_account, nullif(p_buyer_name, ''), nullif(p_buyer_tax_id, ''),
    nullif(p_buyer_address, ''), null, 0, v_rate, 0, 0, coalesce(nullif(p_currency, ''), 'EUR'), v_due
